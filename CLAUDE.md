...
`--per-stage offload-all-mlp,offload-all-mlp,no-ac,no-ac` is the escape hatch for manual experiments: overrides `--ac` and runs exactly the listed strategies per rank. Validated by `parse_per_stage_override()` (length must equal `--pp`; entries must be one of the runner's uniform strategies). The end-of-run simulator comparison routes through `simulate_pipeline_custom_ac(strategy_assignments=...)` so the predicted numbers describe the same assignment that ran on GPU.
...
**Long-context sweet spot on 4× H200 (Llama-7B, seq=32768, mbs=1, μb=8):** No AC OOMs (sim predicts 208 GB/stage > 141 GB HBM). Full AC 15,384 ms / 17,040 tok/s at 27 GB peak. Uniform Offload all MLP 13,478 ms / 19,450 tok/s at 103 GB peak. Pipeline-aware picks `[offload-all-mlp, offload-all-mlp, no-ac, no-ac]` — offload only on stages 0-1 which carry 1F1B stash pressure (3 and 2 microbatches), no-ac on stages 2-3 (stash 1 and 0) — and measures 12,837 ms / 20,420 tok/s at 105 GB peak on stage 2, **+5.0% faster than uniform offload**. Stages 2 & 3 report 0 GB/step of offload traffic exactly as predicted. Simulator predicts 16,473 ms (bubble=37.5%) for both uniform and pipeline-aware because the bottleneck-stage cost is strategy-invariant; the measured 5% gain comes from downstream stages finishing faster and shrinking the 1F1B drain tail, which the current bubble model doesn't capture.

**Longer context on 4× H200 (Llama-7B, seq=65536, mbs=1, μb=8):** Uniform Offload all MLP now OOMs on rank 0 (139.4 / 141 GB used) — stage 0's 1F1B stash of 3 offloaded microbatches saturates HBM at this sequence length. Full AC fits at 47 GB peak, 42,909 ms / 12,218 tok/s. Pipeline-aware picks a **3-level split** `[full-ac, full-ac, offload-all-mlp, no-ac]` and measures 39,592 ms / 13,242 tok/s at 116 GB peak on stage 3, **+7.7% faster than Full AC** — the first 3-level heterogeneous assignment measured on this hardware. Generalizable framing: pipeline-aware expands the feasible configuration region beyond any uniform strategy; the OOM is specific to this (model × HBM × PP × schedule × seq × μb) combination, not a universal claim. Simulator prediction 67,530 ms is 41% pessimistic at seq=64K — relative ranking is still correct but absolute accuracy degrades with sequence length (+1.7% err at seq=4K → −22% at seq=32K → −41% at seq=64K).
